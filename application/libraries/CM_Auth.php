...
class CM_Auth
{
    ...
    public function check()
    {
        ...
        if (isset($_user_logged) && !empty($_user_logged)) {
            ...
            $user = $this->CI->db->select('id,username,password,salt,email,fullname,permission,money,avatar')->where(array('username' => $cookie['username']))->from('user')->get()->row_array();
            ...
            $users=$this->CI->db->select('fullname,id,email')->get('user')->result();
